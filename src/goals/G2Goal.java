...
public class G2Goal extends Goal {
	public boolean isCompleted(RubiksCubeModel cube) {
	    Color LUB = cube.getColor(Face.LEFT.mapping, 0, 0);
	    Color LUF = cube.getColor(Face.LEFT.mapping, 0, 2);
	    Color LDB = cube.getColor(Face.LEFT.mapping, 2, 0);
	    Color LDF = cube.getColor(Face.LEFT.mapping, 2, 2);
	
	    Color RUB = cube.getColor(Face.RIGHT.mapping, 0, 2);
	    Color RUF = cube.getColor(Face.RIGHT.mapping, 0, 0);
	    Color RDB = cube.getColor(Face.RIGHT.mapping, 2, 2);
	    Color RDF = cube.getColor(Face.RIGHT.mapping, 2, 0);
	
	    // Edges in the M slice (between R and L).
	    Color UF = cube.getColor(Face.UP.mapping,    2, 1);
	    Color FU = cube.getColor(Face.FRONT.mapping, 0, 1);
	
	    Color UB = cube.getColor(Face.UP.mapping,    0, 1);
	    Color BU = cube.getColor(Face.BACK.mapping,  0, 1);
	
	    Color DF = cube.getColor(Face.DOWN.mapping,  0, 1);
	    Color FD = cube.getColor(Face.FRONT.mapping, 2, 1);
	
	    Color DB = cube.getColor(Face.DOWN.mapping,  2, 1);
	    Color BD = cube.getColor(Face.BACK.mapping,  2, 1);
